...
const darwin_hack = @import("../darwin.zig");
...
fn AsyncMachPort(comptime xev: type) type {
    return struct {
        ...
        extern "c" fn mach_port_set_attributes(
            ...
            info: *anyopaque,
            count: darwin_hack.mach_msg_type_number_t,
        ...
        pub fn init() !Self {
            ...
            switch (darwin_hack.getKernError(posix.system.mach_port_allocate(
                mach_self,
                @intFromEnum(darwin_hack.MACH_PORT_RIGHT.RECEIVE),
                &mach_port,
            ))) {
                .SUCCESS => {}, // Success
            ...
            switch (darwin_hack.getKernError(posix.system.mach_port_insert_right(
                mach_self,
                mach_port,
                mach_port,
                @intFromEnum(darwin_hack.MACH_MSG_TYPE.MAKE_SEND),
            ))) {
                .SUCCESS => {}, // Success
                else => return error.MachPortAllocFailed,
            }
            ...
            switch (darwin_hack.getKernError(mach_port_set_attributes(
                mach_self,
                mach_port,
        ...
        fn drain(port: posix.system.mach_port_name_t) void {
            var message: struct {
                header: darwin_hack.mach_msg_header_t,
            } = undefined;

            while (true) {
                switch (darwin_hack.getMachMsgError(darwin_hack.mach_msg(
                    &message.header,
                    darwin_hack.MACH_MSG_OPTION.RCV_MSG | darwin_hack.MACH_MSG_OPTION.RCV_TIMEOUT,
                    0,
                    @sizeOf(@TypeOf(message)),
                    port,
                    darwin_hack.MACH_MSG_TIMEOUT_NONE,
                    darwin_hack.MACH_PORT_NULL,
                ))) {
        ...
        pub fn notify(self: Self) !void {
            ...
            var msg: darwin_hack.mach_msg_header_t = .{
                // We use COPY_SEND which will not increment any send ref
                // counts because it'll reuse the existing send right.
                .msgh_bits = @intFromEnum(darwin_hack.MACH_MSG_TYPE.COPY_SEND),
                .msgh_size = @sizeOf(darwin_hack.mach_msg_header_t),
                .msgh_remote_port = self.port,
                .msgh_local_port = darwin_hack.MACH_PORT_NULL,
                .msgh_voucher_port = undefined,
                .msgh_id = undefined,
            };

            return switch (darwin_hack.getMachMsgError(
                darwin_hack.mach_msg(
                    &msg,
                    darwin_hack.MACH_MSG_OPTION.SEND_MSG | darwin_hack.MACH_MSG_OPTION.SEND_TIMEOUT,
                    msg.msgh_size,
                    0,
                    darwin_hack.MACH_PORT_NULL,
